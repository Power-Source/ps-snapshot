			...
			<div class="wps-menu">
				...
				<div class="wps-menu-holder">
					...
					<ul class="wps-menu-list">
						...
						<li>
							<a href="<?php echo PSOURCESnapshot::instance()->snapshot_get_pagehook_url( 'snapshots-newui-snapshots' ); ?>&amp;snapshot-action=edit&amp;item=<?php echo $item['timestamp']; ?>"><?php _e( 'Bearbeiten', SNAPSHOT_I18N_DOMAIN ); ?></a>
						</li>
						<li>
							<a href="<?php echo PSOURCESnapshot::instance()->snapshot_get_pagehook_url( 'snapshots-newui-snapshots' ); ?>&amp;snapshot-action=backup&amp;item=<?php echo $item['timestamp']; ?>"><?php _e( 'Neu generieren', SNAPSHOT_I18N_DOMAIN ); ?></a>
						</li>
						<?php if ( isset( $item['data_item']['timestamp'] ) && ! empty( $item['data_item']['timestamp'] ) ): ?>
							<li>
								<a href="<?php echo PSOURCESnapshot::instance()->snapshot_get_pagehook_url( 'snapshots-newui-snapshots' ); ?>&snapshot-action=restore&item=<?php echo $item['timestamp']; ?>&snapshot-data-item=<?php echo $item['data_item']['timestamp']; ?>"><?php _e( 'Wiederherstellen', SNAPSHOT_I18N_DOMAIN ); ?></a>
							</li>
						<?php endif; ?>
						<li>
							<a href="<?php echo PSOURCESnapshot::instance()->snapshot_get_pagehook_url( 'snapshots-newui-snapshots' ); ?>&amp;snapshot-action=delete-item&amp;item=<?php echo $item['timestamp']; ?>&amp;snapshot-noonce-field=<?php echo wp_create_nonce( 'snapshot-delete-item' ); ?>"><?php _e( 'Löschen', SNAPSHOT_I18N_DOMAIN ); ?></a>
						</li>
			...
			<div class="row">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					...
					<table class="has-footer" cellpadding="0" cellspacing="0">
								...
								<p>
									<?php if ( isset( $item['data_item']['timestamp'] ) ) {
										...
										printf( '<a href="%s" title="%s">%s</a>',
											...
											esc_attr__( 'Lade das Snapshot-Archiv herunter', SNAPSHOT_I18N_DOMAIN ),
						...
						<tr>
							<th><?php _e( 'Letzter Lauf', SNAPSHOT_I18N_DOMAIN ); ?></th>
							<td>
								<p>
									<?php
							...
							<td>
								...
								if ( isset( $destination_status ) && $destination_status['errorStatus'] ) {
									...
									if ( $destination_status['errorArray'] ) {
										...
										echo '<p>', __( 'Bei dem letzten Upload-Versuch ist ein Fehler aufgetreten:', SNAPSHOT_I18N_DOMAIN ), '</p>';

										foreach ( $destination_status['errorArray'] as $error_message ) {
											echo '<p class="wps-auth-message error">', esc_html( $error_message ), '</p>';
										}

										echo '<p>', __( 'Weitere Versuche zum Hochladen werden weiterhin unternommen. Sie sollten jedoch dieses Problem untersuchen, um sicherzustellen, dass sie erfolgreich sind.', SNAPSHOT_I18N_DOMAIN ), '</p>';

									} else {
										esc_html_e( 'Bei dem letzten Upload-Versuch ist ein unbekannter Fehler aufgetreten. Weitere Versuche zum Hochladen werden weiterhin unternommen.', SNAPSHOT_I18N_DOMAIN );
									}

								} else {

									echo $uploaded ?
										'<p>' . __( 'Hochgeladen', SNAPSHOT_I18N_DOMAIN ) . '</p>' :
										'<p class="wps-spinner">' . __( 'Wird hochgeladen&hellip;', SNAPSHOT_I18N_DOMAIN ) . '</p>';
						...
						<tr>
							<th><?php _e( 'Frequenz', SNAPSHOT_I18N_DOMAIN ); ?></th>
							<td>
								<p>
									<?php
									...
									if ( $interval_text ) {
										...
										_e( 'Einmalig', SNAPSHOT_I18N_DOMAIN );
									}
									?>
								</p>
						...
						<tr>
							<th><?php _e( 'Dateien', SNAPSHOT_I18N_DOMAIN ); ?></th>
							<td>
								<p><?php if ( isset( $item['files-option'] ) ) {
										if ( $item['files-option'] == 'none' ) {
											_e( 'Keine', SNAPSHOT_I18N_DOMAIN );
										} else if ( $item['files-option'] == 'all' ) {
											_e( 'Alle Dateien', SNAPSHOT_I18N_DOMAIN );
										} else {
						...
						<tr>
							<th><?php _e( 'URL-Ausschlüsse', SNAPSHOT_I18N_DOMAIN ); ?></th>
							<td>
								<p>
									<?php
						...
						<tr>
							<th><?php _e( 'Datenbanktabellen', SNAPSHOT_I18N_DOMAIN ); ?></th>
							<td>
								<p>
									<?php
						...
						<?php if ( isset( $item['data_item']['timestamp'] ) && ! empty( $item['data_item']['timestamp'] ) ): ?>
							<tr>
								...
								<td>
									...
									<a id="wps-snapshot-log-view" class="button button-outline button-gray" href="#"><?php _e( 'ansehen', SNAPSHOT_I18N_DOMAIN ) ?></a>
									<a class="button button-outline button-gray" href="<?php echo '?page=snapshot_snapshots&amp;snapshot-action=download-log&amp;snapshot-item=' . $item['timestamp'] . '&amp;snapshot-data-item=' . $item['data_item']['timestamp'] . '&amp;live=0' ?>"><?php _e( 'herunterladen', SNAPSHOT_I18N_DOMAIN ) ?>
									</a>
						...
						<tr>
							<td>
								...
								<a href="<?php echo PSOURCESnapshot::instance()->snapshot_get_pagehook_url( 'snapshots-newui-snapshots' ); ?>&amp;snapshot-action=delete-item&amp;item=<?php echo $item['timestamp']; ?>&amp;snapshot-noonce-field=<?php echo wp_create_nonce( 'snapshot-delete-item' ); ?>" class="button button-outline button-gray"><?php _e( 'Löschen', SNAPSHOT_I18N_DOMAIN ); ?></a>
							...
							<td>
								...
								<?php if ( isset( $item['data_item']['timestamp'] ) && ! empty( $item['data_item']['timestamp'] ) ): ?>
									<a class="button button-blue" href="<?php
										...
										<?php _e( 'Wiederherstellen', SNAPSHOT_I18N_DOMAIN ); ?>
									</a>
								<?php endif; ?>
					...
					<?php if ( isset( $item['data_item']['timestamp'] ) && ! empty( $item['data_item']['timestamp'] ) ): ?>
						...
						$modal_data = array(
							...
							'modal_title' => __( 'Protokolle anzeigen', SNAPSHOT_I18N_DOMAIN ),
							'modal_content' => __( "<p>Hier ist ein Protokoll der Ereignisse für diesen Snapshot.</p>", SNAPSHOT_I18N_DOMAIN ),
							...
							'modal_action_title' => __( 'Herunterladen', SNAPSHOT_I18N_DOMAIN ),
							...
							'modal_cancel_title' => __( 'Abbrechen', SNAPSHOT_I18N_DOMAIN ),
							'modal_cancel_url' => '#',
						);
